...
import { fetchPublishedRequisitionsWithShortlisted, createSchedule, deleteSchedule } from './ApiService';
...
const Schedule = () => {
  ...
  // Base URL for the dashboard link
  const WEB_PAGE__URL = config.WEB_PAGE__URL || 'http://localhost:5173';
    ...
    try {
      ...
      for (const applicantId of tempSelectedApplicants) {
        ...
        const scheduleData = {
          ...
          message: `Dear ${applicant.name}, your interview for the ${activeJob.title} role has been scheduled for ${formatFullDate(tempSelectedDate)} at ${formatTime(tempStartTime)}. 
          ${meetingMode === 'Virtual' ? `Join the virtual meeting here: ${meetingLink}` : `Location: ${interviewAddress}`}.
          Please check your application dashboard for further details: ${dashboardLink}. We look forward to speaking with you!`,
        };
            ...
            >
              ...
              <div className="GGtg-DDDVa dd-none">
                ...
                <textarea
                  ...
                  value={`Dear ${tempSelectedApplicants.length === 1 ? currentApplicants.find((app) => app.id === tempSelectedApplicants[0])?.name || 'Applicant' : 'Applicant'}, your interview for the ${activeJob.title} role has been scheduled for ${formatFullDate(tempSelectedDate)} at ${formatTime(tempStartTime)}. ${meetingMode === 'Virtual' ? `Join the virtual meeting here: ${meetingLink}` : `Location: ${interviewAddress}`}. Please check your application dashboard for further details: ${WEB_PAGE__URL}/application-dashboard/${tempSelectedApplicants.length === 1 ? currentApplicants.find((app) => app.id === tempSelectedApplicants[0])?.id : '[job_application_code]'}/${encodeURIComponent(tempSelectedApplicants.length === 1 ? currentApplicants.find((app) => app.id === tempSelectedApplicants[0])?.email : '[email]')}/${activeJob.unique_link}. We look forward to speaking with you!`}
                  readOnly
                />
              </div>
...
//           message: `
// Dear ${applicant.name},
// Your interview for the ${activeJob.title} role has been scheduled for ${formatFullDate(tempSelectedDate)} at ${formatTime(tempStartTime)}.
// ${meetingMode === 'Virtual' ? `Join the virtual meeting here: <a href="${meetingLink}" style="color: #7226FF; text-decoration: underline;">${meetingLink}</a>` : `Location: ${interviewAddress}`}.
// Please check your <a href="${dashboardLink}" style="color: #7226FF; text-decoration: underline;">application dashboard</a> for further details.
...
//                   className="oujka-Inpuauy OIUja-Tettxa"
//                   value={`
// Dear ${tempSelectedApplicants.length === 1 ? currentApplicants.find((app) => app.id === tempSelectedApplicants[0])?.name || 'Applicant' : 'Applicant'},
// Your interview for the ${activeJob.title} role has been scheduled for ${formatFullDate(tempSelectedDate)} at ${formatTime(tempStartTime)}.
// ${meetingMode === 'Virtual' ? `Join the virtual meeting here: <a href="${meetingLink}" style="color: #7226FF; text-decoration: underline;">${meetingLink}</a>` : `Location: ${interviewAddress}`}.
// Please check your <a href="${WEB_PAGE__URL}/application-dashboard/${tempSelectedApplicants.length === 1 ? currentApplicants.find((app) => app.id === tempSelectedApplicants[0])?.id : '[job_application_code]'}/${encodeURIComponent(tempSelectedApplicants.length === 1 ? currentApplicants.find((app) => app.id === tempSelectedApplicants[0])?.email : '[email]')}/${activeJob.unique_link}" style="color: #7226FF; text-decoration: underline;">application dashboard</a> for further details.
// We look forward to speaking with you!
//                   `}
//                   readOnly
//                 />
