    ...
    if (job?.id) {
      axios
        ...
        .then((response) => {
          ...
          // Update form data with requisition and advert fields
          setFormData({
            jobTitle: data.title || '',
          ...
          if (data.advert_banner) {
            setAdvertBanner(`${config.API_BASE_URL}${data.advert_banner}`);
          }
        })
        .catch((error) => {
          if (error.response?.status !== 404) {
...
const handlePublish = async () => {
  ...
  const formDataToSend = new FormData();
  formDataToSend.append('title', formData.jobTitle); // Ensure title is included
  ...
  console.log('FormData to send:', formDataEntries);

  try {
    await axios({
      method: 'patch',
      url: `${API_BASE_URL}/api/talent-engine/requisitions/${job.id}/`,
      data: formDataToSend,
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'multipart/form-data',
    ...
    let errorMessage = 'Failed to publish job advert. Please try again.';
    ...
    console.error('Error publishing advert:', error.response?.data || error);
  }
};
...
const handleReject = async () => {
  ...
  const confirmDeleteAdvert = async () => {
    try {
      // Delete requisition (includes advert data)
      await axios.delete(`${API_BASE_URL}/api/talent-engine/requisitions/${job.id}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setShowDeleteModal(false);
