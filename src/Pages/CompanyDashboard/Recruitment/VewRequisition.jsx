...
// Modal component
const Modal = ({ title, message, onConfirm, onCancel, confirmText = 'Confirm', cancelText = 'Cancel', children }) => (
  <AnimatePresence>
    <Backdrop onClick={onCancel} />
    <motion.div
      className="fixed top-1/2 left-1/2 z-50 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-6 shadow-lg"
...
const VewRequisition = ({ job, onClose }) => {
  ...
  const [documentTitle, setDocumentTitle] = useState('');
  const [documentTitleError, setDocumentTitleError] = useState('');
  const [complianceItemError, setComplianceItemError] = useState('');
  const [showEditComplianceModal, setShowEditComplianceModal] = useState(false);
  const [editComplianceItem, setEditComplianceItem] = useState({
    id: '',
    name: '',
    description: '',
    required: true,
      ...
      const fetchData = async () => {
        try {
         ...
         // console.log('Fetched requisition data:', data); // Debug log
          ...
          setComplianceItems(data.compliance_checklist || []);
  ...
  const handleAddComplianceItem = async () => {
    ...
    if (!newComplianceItem.name.trim()) {
      setComplianceItemError('Compliance item name is required');
      return;
    }

    // Check for uniqueness
    if (complianceItems.some(item => item.name.toLowerCase() === newComplianceItem.name.trim().toLowerCase())) {
      setComplianceItemError('Compliance item name must be unique');
      return;
    }

    try {
      const itemData = {
        name: newComplianceItem.name.trim(),
        description: newComplianceItem.description.trim(),
        required: newComplianceItem.required,
        checked_by: null,
      };
      const response = await createComplianceItem(job.id, itemData);
      //console.log('Add compliance item response:', response); // Debug log
      setComplianceItems(prev => [...prev, response]);
      setNewComplianceItem({ name: '', description: '', required: true });
      setComplianceItemError('');
      setErrors(prev => ({ ...prev, compliance: '' }));
      setShowAddComplianceInput(false);
    } catch (error) {
      setComplianceItemError(error.response?.data?.detail || error.message);
      console.error('Error adding compliance item:', error);
    }
  };

const handleUpdateComplianceItem = async (itemId, updatedData) => {
    if (!isFormMutable) return;
    try {
       // console.log('Updating compliance item:', { itemId, updatedData }); // Debug log
        const response = await updateComplianceItem(job.id, itemId, updatedData);
       // console.log('Update compliance item response:', response); // Debug log
        setComplianceItems(prev =>
            prev.map(item => (String(item.id) === String(itemId) ? { ...item, ...response } : item))
        );
        setShowEditComplianceModal(false);
        setEditComplianceItem({ id: '', name: '', description: '', required: true });
        setComplianceItemError('');
        setErrors(prev => ({ ...prev, compliance: '' }));
        showAlert('Success', 'Compliance item updated successfully.');
    } catch (error) {
        console.error('Error updating compliance item:', error);
        const errorMessage = error.response?.data?.detail || error.message || 'Failed to update compliance item.';
        setComplianceItemError(errorMessage);
        showAlert('Error', errorMessage);
    }
};

  const handleRemoveComplianceItem = async (itemId) => {
    if (!isFormMutable) return;
    try {
      //console.log('Deleting compliance item:', itemId); // Debug log
      await deleteComplianceItem(job.id, itemId);
      setComplianceItems(prev => prev.filter(item => String(item.id) !== String(itemId)));
      setErrors(prev => ({ ...prev, compliance: '' }));
    } catch (error) {
      console.error('Error removing compliance item:', error);
      setComplianceItemError(error.response?.data?.detail || error.message);
  ...
  const handlePublish = async () => {
    ...
    if (complianceItems.length === 0) {
      setErrors({ compliance: 'At least one compliance item is required' });
      showAlert('Compliance Error', 'Please add at least one compliance item');
      return;
    }
    ...
    formDataToSend.append('documents_required', JSON.stringify(documents));
    formDataToSend.append('compliance_checklist', JSON.stringify(complianceItems));
    if (formData.advertBannerFile) {
      formDataToSend.append('advert_banner', formData.advertBannerFile);
    }
    formDataToSend.append('publish_status', 'true');
  ...
  const handleTabClick = (index) => {
    ...
    if (index > activeSection) {
      if (activeSection === 0) {
        ...
        showAlert('Document Error', 'Please add at least one document title');
        return;
      } else if (activeSection === 2 && complianceItems.length === 0) {
        setErrors({ compliance: 'At least one compliance item is required' });
        showAlert('Compliance Error', 'Please add at least one compliance item');
        return;
      }
    ...
    <div className='VewRequisition'>
                          ...
                          >
                            <div className='GHuh-Form-Input'>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setNewComplianceItem(prev => ({ ...prev, name: e.target.value }))
                                }
                                required
                                disabled={!isFormMutable}
                              />
                              ...
                              <textarea
                                ...
                                onChange={(e) =>
                                  setNewComplianceItem(prev => ({ ...prev, description: e.target.value }))
                                }
                                disabled={!isFormMutable}
                              /> */}
                              <label>
                                <input
                                  ...
                                  onChange={(e) =>
                                    setNewComplianceItem(prev => ({ ...prev, required: e.target.checked }))
                                  }
                                  disabled={!isFormMutable}
                                />
                                Required
                              ...
                              {complianceItemError && <p className='error'>{complianceItemError}</p>}
                            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
                            ...
                            >
                              ...
                              <div>
                                <button
                                  ...
                                  onClick={() => {
                                    setEditComplianceItem({ id: item.id, name: item.name, description: item.description || '', required: item.required });
                                    setShowEditComplianceModal(true);
                                  }}
                                  disabled={!isFormMutable}
                                >
                                  <PencilIcon className='w-4 h-4' />QWERTY
                                </button>
                                <button
                                  className="remove-compliance-btn"
      ...
      {showEditComplianceModal && (
        <Modal
          ...
          message="Update the compliance item details below."
          onConfirm={async () => {
            if (!editComplianceItem.name.trim()) {
              setComplianceItemError('Compliance item name is required');
              return;
            }
            if (complianceItems.some(item => item.id !== editComplianceItem.id && item.name.toLowerCase() === editComplianceItem.name.trim().toLowerCase())) {
              setComplianceItemError('Compliance item name must be unique');
              return;
            }
            try {
              const updatedData = {
                name: editComplianceItem.name.trim(),
                description: editComplianceItem.description.trim(),
                required: editComplianceItem.required,
                checked_by: null,
              };
              //console.log('Submitting update for compliance item:', updatedData); // Debug log
              await handleUpdateComplianceItem(editComplianceItem.id, updatedData);
              setShowEditComplianceModal(false);
              setEditComplianceItem({ id: '', name: '', description: '', required: true });
              setComplianceItemError('');
            } catch (error) {
              console.error('Error in edit modal:', error); // Debug log
              setComplianceItemError(error.message);
            }
          }}
          onCancel={() => {
            setShowEditComplianceModal(false);
        ...
        >
          <div className='GHuh-Form-Input'>
            ...
            <input
              type='text'
              placeholder='Enter Compliance Item Name'
              value={editComplianceItem.name}
              onChange={(e) =>
                setEditComplianceItem(prev => ({ ...prev, name: e.target.value }))
              }
              required
              disabled={!isFormMutable}
            />
            ...
            <textarea
              placeholder='Enter Description'
              value={editComplianceItem.description}
              onChange={(e) =>
                setEditComplianceItem(prev => ({ ...prev, description: e.target.value }))
              }
              disabled={!isFormMutable}
            />
            <label>
              <input
                type='checkbox'
                checked={editComplianceItem.required}
                onChange={(e) =>
                  setEditComplianceItem(prev => ({ ...prev, required: e.target.checked }))
                }
                disabled={!isFormMutable}
              />
              Required
